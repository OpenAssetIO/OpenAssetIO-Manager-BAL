...
def related_references(
    ...
    requested_relation_traits: TraitsData,
    ...
    for relation in entity_data.relations:
        ...
        if result_trait_set:
            for related_entity_info in relation.entity_infos:
                if _entity_has_trait_set(related_entity_info, result_trait_set, library):
                    results.append(related_entity_info)
        else:
            results.extend(relation.entity_infos)
...
def _dict_has_traits(data: dict, traits: TraitsData) -> bool:
    """
    ...
    """
    for trait_id in traits.traitSet():
        if trait_id not in data:
            return False
        trait_data = data[trait_id]
        for property_key in traits.traitPropertyKeys(trait_id):
            if trait_data.get(property_key, None) != traits.getTraitProperty(
...
def _entity_has_trait_set(entity_info: EntityInfo, trait_set: Set[str], library: dict) -> bool:
    """
    Determines if the entity referenced by the supplied EntityInfo has
    the requested trait ids within its trait set.
    """
    entity_data = entity(entity_info, library)
